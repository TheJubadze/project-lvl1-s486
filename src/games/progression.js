...
const getGameObject = () => {
  ...
  // Комментарий ментора: Модуль lodash у вас расположен в секции для разработки.
  // При загрузке пакета пользователем эта функция не заработает.
  // Не могли бы, пожалуйста, объянить подробнее, в каком месте здесь у меня
  // используется модуль lodash? Я старался обойтись лишь прототипом Array.
  // А в pachage.json прописан только eslint-plugin-lodash-fp. Вы его имеете в виду? Спасибо.
  // ЗЫ: Подскажите, пожалуйста, куда можно писать вопросы по шагу "Ментор", ибо там отсутствует
  // форма "Мои вопросы". Спасибо.
